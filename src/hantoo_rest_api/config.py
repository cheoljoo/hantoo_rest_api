...
class KisConfig:
    ...
    한투 Open API 신청 시에는 계좌 1개를 선택해 인증하지만(README 참고), 실제로는
    같은 HTS ID에 딸린 여러 계좌(일반/퇴직연금/ISA 등)를 **같은 앱키/앱시크릿**으로
    CANO/ACNT_PRDT_CD만 바꿔가며 조회할 수 있다 — 계좌마다 별도 앱을 등록할 필요는
    없다(실사용자 확인). 그래서 `load_configs()`는 두 번째 계좌부터 앱키/앱시크릿을
    생략하면 첫 번째 계좌 것을 그대로 재사용하고, 계좌번호(및 선택적으로 상품코드)만
    다르게 지정하면 되도록 만들어졌다.
...
def _load_one(suffix: str, *, fallback_app_key: str = "", fallback_app_secret: str = "") -> KisConfig | None:
    """`suffix`(빈 문자열 또는 "_2", "_3", ...)가 붙은 환경변수 세트 하나를 읽는다.

    이 계좌 존재 여부는 `KIS_ACCOUNT_NO{suffix}`만으로 판단한다 — 앱키/앱시크릿은
    같은 HTS ID의 여러 계좌가 공유하는 경우가 많아(실사용자 확인), suffix별로
    지정하지 않으면 `fallback_app_key`/`fallback_app_secret`(보통 첫 번째 계좌
    값)을 그대로 쓴다. `load_configs()`는 계좌번호가 없으면 None을 받아 "더 이상
    다음 번호 계좌가 없다"는 신호로 순회를 멈춘다.
    """
    account_no = os.environ.get(f"KIS_ACCOUNT_NO{suffix}", "")
    if not account_no:
        return None

    app_key = os.environ.get(f"KIS_APP_KEY{suffix}", "") or fallback_app_key
    app_secret = os.environ.get(f"KIS_APP_SECRET{suffix}", "") or fallback_app_secret
...
def load_configs(*, max_accounts: int = 10) -> list[KisConfig]:
    ...
    첫 번째 계좌는 접미사 없는 변수(`KIS_APP_KEY`/`KIS_ACCOUNT_NO` 등)로 읽는다.
    두 번째부터는 `KIS_ACCOUNT_NO_2`, `KIS_ACCOUNT_NO_3`, ... 형식의 번호
    접미사로 계좌번호(및 선택적으로 상품코드/모의여부/표시이름)만 추가하면
    되고, 앱키/앱시크릿은 (`KIS_APP_KEY_2`처럼) 별도로 지정하지 않는 한 첫
    번째 계좌 것을 그대로 재사용한다. 번호가 중간에 비어있으면(예: `_2`는
    없는데 `_3`은 있는 경우) 그 지점에서 순회를 멈춘다 — 설정 실수를 조용히
    넘어가지 않기 위함이다. `max_accounts`는 무한 루프 방지용 상한이며,
    실사용에서 이만큼 계좌를 등록할 일은 거의 없다.
    ...
    for i in range(2, max_accounts + 1):
        cfg = _load_one(
            f"_{i}", fallback_app_key=first.app_key, fallback_app_secret=first.app_secret
        )
        if cfg is None:
            break
        configs.append(cfg)
