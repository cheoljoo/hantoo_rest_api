...
class KisConfig:
    ...
    표시는 필요 없음, README 참고), 앱키/앱시크릿이 계좌 하나에 고정된다. 계좌를
    여러 개 쓰려면(예: 일반 위탁계좌 + 퇴직연금 계좌) 계좌마다 별도로 앱을
    등록하고 별도의 `KisConfig`를 만들어야 한다 — `load_configs()`가 이 다중
    계좌 설정을 담당한다.
...
def load_configs(*, max_accounts: int = 10) -> list[KisConfig]:
    ...
    `KIS_APP_KEY_2`, `KIS_APP_KEY_3`, ... 형식의 번호 접미사로 읽는다. 번호가
    중간에 비어있으면(예: `_2`는 없는데 `_3`은 있는 경우) 그 지점에서 순회를
    멈춘다 — 설정 실수를 조용히 넘어가지 않기 위함이다. `max_accounts`는
    무한 루프 방지용 상한이며, 실사용에서 이만큼 계좌를 등록할 일은 거의 없다.
